...
fn cpu_fft_concurrent(domain_size: usize, buffers: &[Vec<u64>]) -> Vec<Vec<u64>> {
    let start = std::time::Instant::now();

    let mut result = vec![];
    for buffer in buffers {
        let chunks: Vec<_> = buffer.chunks(domain_size).collect();
        let a: Vec<_> = chunks.par_iter().flat_map(|chunk|{
            let coeffs = chunk
                .iter()
...
#[cfg(not(feature = "no_cuda"))]
fn gpu_fft_batch_multiple_devices(log_ntt_size: usize, buffers: &mut [Vec<u64>]) {
    let start = std::time::Instant::now();
...
fn main() {
    ...
    let cpu_result = cpu_fft_concurrent(domain_size, &gpu_buffers);
    ...
    #[cfg(not(feature = "no_cuda"))]
    gpu_fft_batch_multiple_devices(log_ntt_size, &mut gpu_buffers);
