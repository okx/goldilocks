...
testsgpu: tests/tests.cpp $(ALLSRCS)
	...
	$(NVCC) -D__USE_CUDA__ _DFEATURE_GOLDILOCKS -Xcompiler -fopenmp -Xcompiler -fPIC -Xcompiler -mavx2 cryptography_cuda/src/lib.cu -Icryptography_cuda/cuda -Icryptography_cuda/depends/blst/src -arch=$(CUDA_ARCH) -g -dc --output-file ntt_gpu.o
	...
	$(NVCC) -Xcompiler -fopenmp -arch=$(CUDA_ARCH) -g -o $@ tests.o util.o goldilocks_base_field.o goldilocks_cubic_extension.o ntt_goldilocks.o poseidon_goldilocks.o poseidon_goldilocks_gpu.o ntt_gpu.o -lgtest -lgmp
