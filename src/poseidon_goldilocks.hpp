...
public:
    ...
    void static merkletree_cuda_multi_gpu_full2(Goldilocks::Element *tree, uint64_t** gpu_inputs, uint64_t** gpu_trees, void* gpu_streams, uint64_t num_cols, uint64_t num_cols_device, uint64_t num_rows, uint64_t num_rows_device, uint32_t const ngpu, uint64_t dim = 1);
    void static merkletree_cuda_multi_gpu_full_um(Goldilocks::Element *tree, uint64_t** gpu_inputs, uint64_t** gpu_trees, void* gpu_streams, uint64_t num_cols, uint64_t num_rows, uint64_t num_rows_device, uint32_t const ngpu, uint64_t dim = 1);
    void static merkletree_cuda_multi_gpu_full_um2(Goldilocks::Element *tree, uint64_t** gpu_inputs, uint64_t** gpu_trees, void* gpu_streams, uint64_t num_cols, uint64_t num_cols_device, uint64_t num_rows, uint64_t num_rows_device, uint32_t const ngpu, uint64_t dim = 1);
#endif
